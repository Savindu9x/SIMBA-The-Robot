...
void clk3_Start(void) 
{
    ...
    clk3_CLKEN |= clk3_CLKEN_MASK;
	clk3_CLKSTBY |= clk3_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: clk3_Stop
********************************************************************************
*
* Summary:
...
void clk3_Stop(void) 
{
    ...
    clk3_CLKEN &= (uint8)(~clk3_CLKEN_MASK);
	clk3_CLKSTBY &= (uint8)(~clk3_CLKSTBY_MASK);
}
...
{
    if ((clk3_CLKEN & clk3_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = clk3_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = clk3_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(clk3_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(clk3_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: clk3_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: clk3_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = clk3_GetSourceRegister();
    uint16 oldDivider = clk3_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = clk3_CLKEN & clk3_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((clk3_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: clk3_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(clk3_DIV_PTR);
}


/*******************************************************************************
* Function Name: clk3_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    clk3_MOD_SRC |= modeBitMask & (uint8)clk3_MODE_MASK;
}


/*******************************************************************************
* Function Name: clk3_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    clk3_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(clk3_MODE_MASK));
}


/*******************************************************************************
* Function Name: clk3_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return clk3_MOD_SRC & (uint8)(clk3_MODE_MASK);
}


/*******************************************************************************
* Function Name: clk3_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = clk3_GetDividerRegister();
    uint8 oldSrc = clk3_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        clk3_MOD_SRC |= CYCLK_SSS;
        clk3_MOD_SRC =
            (clk3_MOD_SRC & (uint8)(~clk3_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: clk3_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return clk3_MOD_SRC & clk3_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: clk3_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    clk3_PHASE = clkPhase & clk3_PHASE_MASK;
}


/*******************************************************************************
* Function Name: clk3_GetPhase
********************************************************************************
*
* Summary:
...
{
    return clk3_PHASE & clk3_PHASE_MASK;
}
