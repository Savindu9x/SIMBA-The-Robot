...
void clk8_Start(void) 
{
    ...
    clk8_CLKEN |= clk8_CLKEN_MASK;
	clk8_CLKSTBY |= clk8_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: clk8_Stop
********************************************************************************
*
* Summary:
...
void clk8_Stop(void) 
{
    ...
    clk8_CLKEN &= (uint8)(~clk8_CLKEN_MASK);
	clk8_CLKSTBY &= (uint8)(~clk8_CLKSTBY_MASK);
}
...
{
    if ((clk8_CLKEN & clk8_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = clk8_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = clk8_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(clk8_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(clk8_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: clk8_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: clk8_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = clk8_GetSourceRegister();
    uint16 oldDivider = clk8_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = clk8_CLKEN & clk8_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((clk8_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: clk8_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(clk8_DIV_PTR);
}


/*******************************************************************************
* Function Name: clk8_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    clk8_MOD_SRC |= modeBitMask & (uint8)clk8_MODE_MASK;
}


/*******************************************************************************
* Function Name: clk8_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    clk8_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(clk8_MODE_MASK));
}


/*******************************************************************************
* Function Name: clk8_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return clk8_MOD_SRC & (uint8)(clk8_MODE_MASK);
}


/*******************************************************************************
* Function Name: clk8_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = clk8_GetDividerRegister();
    uint8 oldSrc = clk8_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        clk8_MOD_SRC |= CYCLK_SSS;
        clk8_MOD_SRC =
            (clk8_MOD_SRC & (uint8)(~clk8_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: clk8_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return clk8_MOD_SRC & clk8_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: clk8_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    clk8_PHASE = clkPhase & clk8_PHASE_MASK;
}


/*******************************************************************************
* Function Name: clk8_GetPhase
********************************************************************************
*
* Summary:
...
{
    return clk8_PHASE & clk8_PHASE_MASK;
}
